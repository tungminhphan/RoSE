...
class Agent:
    ...
    def apply(self, ctrl):
        ...
        # check whether the new state is out of bounds!!
...
class Car(Agent):
    ...
    def set_intention(self):
        ...
        #choice = np.where(scores == np.max(scores))[0][0]
        #print(np.where(scores == np.max(scores))[0])
...
>>>>>>> Stashed changes
  ...
  # assuming ego and all agents in agent_set belong to the same bundle
    ...
    def action_selection_strategy(self):
        ...
        if self.agent_max_braking_not_enough is not None:
            ...
            if agent_type == 'none' and bubble_chk:
                ...
                ctrl = self.get_max_forward_ctrl()
            elif agent_type == 'sender' and bubble_chk and not cluster_chk:
                # TODO: take straight action, best safe one that aligns with intention
                self.token_count = self.token_count+1
                ctrl = self.get_backup_plan_ctrl()
            elif agent_type == 'sender' and bubble_chk and cluster_chk:
                # take intended action as long as it safe w.r.t. agents behind in precedence too
                ctrl = self.intention
                self.token_count = 0
            elif agent_type == 'sender' and not bubble_chk and not cluster_chk:
                # TODO: take straight action, best safe one that aligns with intention
                self.token_count = self.token_count+1
                ctrl = self.get_max_forward_ctrl()
            elif agent_type == 'sender' and not bubble_chk and cluster_chk:
                # TODO: take straight action, best safe one that aligns with intention
                self.token_count = self.token_count+1
                ctrl = self.get_max_forward_ctrl()
                ...
                ctrl = self.intention
                self.token_count = 0
            elif agent_type == 'receiver' and not bubble_chk and not cluster_chk:
                # yield as much as needed for conflict winner to move
    ...
    def compute_gap_req_from_wall(self):
        dx = self.compute_dx(self.a_min, self.state.v)
        gap = dx+1
...
class Game:
    ...
    def check_conflict_requests_karena_debug(self):
        ...
        states = [(27,47, 'east', 1)]
        intentions = [{'acceleration': 3, 'steer': 'straight'}] # {'acceleration': 3, 'steer': 'right-turn' }]
        self.fix_agent_states_karena_debug(states, intentions)
        # check the code for evaluating
    ...
    def play(self, t_end=np.inf, outfile=None, debug=False):
        ...
        if write_bool:
            ...
            traces["collisions_dict"] = self.collisions_dict
...
class Map:
    ...
    def get_IO_map(self):
        ...
        #print('sources')
        #[print(source.node) for source in sources]
        #print('sinks')
        #[print(sink.node) for sink in sinks]
    ...
    def get_sources_sinks(self):
        ...
        for tile in self.legal_orientations:
            if self.legal_orientations[tile]:
                ...
                if is_sink:
                    #if self.legal_orientations[tile] is not None: 
                    sink = Sink(inp)
...
class StaticObstacleOracle(Oracle):
    ...
    def evaluate(self, ctrl_action, plant, game):
        ...
        # check if backup plan would be safe
            tiles = self.get_tiles_for_safety_plan()
            for tile in tiles: 
                if tile not in self.supervisor.game.map.drivable_nodes:
                    return False
            return True
...
class TrafficLight:
    ...
    def set_traffic_light(self, hstate, htimer):
        #traffic_light = self.tile_to_traffic_light_map[traffic_tile]
        assert hstate in ['red', 'yellow', 'green']
...
class QuasiSimultaneousGame(Game):
    ...
    def play_step(self):
        # printing out all agents and states and their goals
        for agent in self.agent_set:
            print(agent.state)
...
def start_game_from_trace(filename, t_index, output_filename):
    ...
    # start the game
    game.play(t_end=5, outfile=output_filename, debug=True)
...
def debug_game(in_filename, output_filename):
    with open(in_filename, 'rb') as pckl_file:
        traces = pickle.load(pckl_file)
...
>>>>>>> Stashed changes
    ...
    # DEBUGGING
    #file_sv = os.getcwd()+'/saved_traces/game.p'
    #debug_game = debug_game(file_sv, output_filename)
    #start_game_from_trace(file_sv, 13, output_filename)
